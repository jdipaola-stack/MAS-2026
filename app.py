...
def tank_figure(model):
    ...
    if model.light_enabled and model.light_field is not None:
        x, y = np.meshgrid(np.linspace(0, model.width, 160), np.linspace(0, model.height, 160))
        brightness = model.light_values(np.stack((x, y), axis=-1))
        field = ax.imshow(brightness, extent=(0, model.width, 0, model.height),
                          origin="lower", cmap="gray", vmin=0, vmax=1, aspect="auto")
        if brightness.min() < 0.25 < brightness.max():
            ax.contour(x, y, brightness, levels=[0.25], colors=["#34d399"], linewidths=1.2)
        ax.text(0.02, 0.97, "MOVING PATCH · green contour: 25% light", transform=ax.transAxes,
                va="top", fontsize=8, color="#065f46", bbox={"facecolor": "white", "alpha": 0.85})
        figure.colorbar(field, ax=ax, label="Light intensity (0 = dark, 1 = bright)", shrink=0.7, pad=0.02)
    elif model.light_enabled:
        ...
        ax.text(0.02, target_y,
                ("Dark region: light ≤ 25%" if model.navigation_mode == "Paper model"
                 else f"Preferred light: {model.preferred_light:.0%}"),
    ...
    figure.colorbar(arrows, ax=ax, label="Swimming speed (BL/s)", shrink=0.7,
                   pad=0.16 if model.light_enabled and model.light_field is None else 0.04)
    unhealthy = np.array([f.health < 0.35 for f in model.fish], dtype=bool)
    if model.navigation_mode == "Informed minority":
        informed = np.array([f.informed for f in model.fish], dtype=bool)
        ax.scatter(model.positions[informed, 0], model.positions[informed, 1],
                   facecolors="none", edgecolors="#f59e0b", s=55, linewidths=1.2)
    elif model.navigation_mode == "Recruitment" and model.signaling.any():
        ax.scatter(model.positions[model.signaling, 0], model.positions[model.signaling, 1],
                   facecolors="none", edgecolors="#22c55e", s=55, linewidths=1.2)
    if unhealthy.any():
...
def SchoolView(model):
    ...
    m = model.metrics
    if model.navigation_mode == "Paper model":
        solara.Markdown(f"**2018 paper model · {model.paper_species} · w = {model.effective_gradient_weight:g}**\n\n"
                        "Shiners change speed with brightness; tetras also sense the local gradient. "
                        "Green marks light ≤ 25%, an observation threshold; the movement rule prefers darkness. "
                        f"**Nearest neighbour: {m['Nearest neighbour (BL)']:.2f} BL** · "
                        f"**Mean darkness: {m['Mean darkness (raw psi)']:.2f}** (raw, not normalized Ψ).")
    elif model.navigation_mode == "Informed minority":
        solara.Markdown(f"**Informed minority:** {m['Informed fish']} informed (orange rings) · "
                        f"{model.num_fish - m['Informed fish']} uninformed. Uninformed fish only respond to neighbours.")
    elif model.navigation_mode == "Recruitment":
        solara.Markdown(f"**Recruitment:** {m['Signaling fish']} fish signaling safe light (green rings). "
                        "Signals are visible only to nearby neighbours.")
    arrivals = model.arrival_summary()
    completion = f"{arrivals['time_all_s']:.1f} s" if arrivals["success"] else "not yet"
    solara.Markdown(f"**Starting fish that reached safety: {arrivals['fraction_arrived']:.0%}** · "
                    f"Last arrival: **{completion}**. Each arrival requires {model.safety_hold_time:g} seconds in the band.")
    solara.Markdown(
    ...
    if model.light_enabled and model.navigation_mode != "Paper model":
        solara.Markdown(
...
def HistoryView(model):
    ...
    if model.light_enabled:
        ...
        bottom.plot(time, frame["Starting cohort reached safety"], label="Starting fish reached safety", color="#7c3aed", linestyle=":")
    ...
    if model.navigation_mode == "Paper model" and not model.lifecycle_enabled:
        population.plot(time, frame["Nearest neighbour (BL)"], label="Nearest neighbour", color="#2563eb")
        population.plot(time, frame["Group radius (BL)"], label="Group radius", color="#d97706")
        population.set(ylabel="Distance (BL)", xlabel="Time (seconds)")
    else:
    ...
    descriptions = {
        "Paper model": "The paper preset fixes a 28 × 28 BL arena, three social zones (0.5 / 3 / 5.5 BL), "
                       "270° vision, a 0.125 s tick, and speeds from 0.2 to 5 BL/s. "
                       "Shiners use w=0; tetras use w=31.6; select Custom weight to use the w control. "
                       "Legacy movement and preferred-light controls apply only to the other experiments. "
                       "The moving noisy patch is an approximation of the paper's light field, so this is not an exact replication. "
                       "Use paper_experiments.py for normalized tracking performance Ψ.",
        "Shared sensing": f"Fish need readings from at least {model.school_min_neighbors} nearby schoolmates to estimate a light direction.",
        "Informed minority": "Only informed fish sense the light field. Uninformed fish follow all neighbours equally; they cannot identify who is informed or change speed based on light.",
        "Recruitment": "All fish sense local brightness but have no direct light-gradient steering. Fish in safe light signal their presence; visible neighbours are attracted to them. Compare with signaling disabled and with independent searches.",
    }
    solara.Markdown(
        ...
        ("The dark patch moves through a noisy light field. " if model.light_field is not None
         else "Light increases from bottom to top. ") + descriptions[model.navigation_mode] + "\n\n"
        "When survival and reproduction are enabled, health falls outside the green area and recovers inside. "
...
MODEL_PARAMS = {
    "navigation_mode": {"type": "Select", "label": "Navigation experiment", "value": "Paper model",
                        "values": ["Paper model", "Informed minority", "Recruitment", "Shared sensing"]},
    "paper_species": {"type": "Select", "label": "Paper sensing mechanism", "value": "Shiners",
                      "values": ["Shiners", "Tetras", "Custom weight"]},
    "paper_habitat": {"type": "Select", "label": "Paper light environment", "value": "Moving patch",
                      "values": ["Moving patch", "Vertical gradient"]},
    "gradient_weight": Slider("Environmental weight w (Custom weight only)", 31.6, 0.0, 100.0, 0.1),
    "gradient_error": Slider("Gradient sensing error (radians)", 0.0, 0.0, 3.14, 0.05),
    "environmental_noise": Slider("Light-field noise η (moving patch)", 0.25, 0.0, 0.5, 0.05),
    "patch_speed": Slider("Dark patch speed (BL/s)", 1.142, 0.0, 3.0, 0.001),
    "informed_fraction": Slider("Fraction informed (minority experiment)", 0.1, 0.0, 1.0, 0.05),
    "recruitment_enabled": {"type": "Checkbox", "label": "Enable recruitment signals", "value": True},
    "recruitment_weight": Slider("Recruitment strength", 2.0, 0.0, 5.0, 0.25),
    "social_enabled": {"type": "Checkbox", "label": "Enable neighbour interactions", "value": True},
    "start_away": {"type": "Checkbox", "label": "Start away from preferred light", "value": True},
    ...
    "lifecycle_enabled": {"type": "Checkbox", "label": "Enable light survival and reproduction", "value": False},
...
def Page():
    ...
    initial_model = solara.use_memo(lambda: GoldenShinerModel(light_enabled=True, navigation_mode="Paper model", start_away=True, lifecycle_enabled=False), dependencies=[])
